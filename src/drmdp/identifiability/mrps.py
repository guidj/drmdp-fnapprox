...
def get_cycle_mrp(n=8):
    """Symmetric random walk on a cycle graph of n states.
...
def get_path_mrp(n=8):
    """Random walk on a path (line) graph of n states with reflecting boundaries.
    ...
    at the far end (R=10 at state n-1), producing a monotonically increasing
...
def get_hypercube_mrp(d=3):
    """Random walk on a d-dimensional binary hypercube {0,1}^d.

    States are the 2^d binary strings of length d. Each step flips one bit
    uniformly at random, so each state has exactly d neighbors. Reward is
    placed at state 0 (the all-zeros vertex). The Hamming distance from state 0
    determines the value gradient.
    ...
    for i in range(n):
        for j in range(d):
            neighbor = i ^ (1 << j)  # Bit-flip defines adjacency
            transition[i, neighbor] = 1.0 / d
    rewards = np.zeros(n)
    rewards[0] = 10
    return transition, rewards
...
def get_complete_mrp(n=8):
    """Random walk on a complete graph of n states (fully connected, no self-loops).

    Each state transitions uniformly to any of the other n-1 states. Reward is
    ...
    URL: https://www.stat.berkeley.edu/~aldous/RWG/book.html
    """
    transition = np.full((n, n), 1.0 / (n - 1))
...
def get_expander_mrp(n=10, d=3, seed: Optional[int] = None):
    """Random walk on a random d-regular-like graph of n states.

    Each state is connected to d randomly chosen neighbors (not necessarily
    ...
    transition = np.zeros((n, n))
    for i in range(n):
        targets = rng.choice([j for j in range(n) if i != j], d, replace=False)
        transition[i, targets] = 1.0 / d
    rewards = rng.uniform(0, 10, n)
    return transition, rewards
